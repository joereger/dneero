...
<%@ include file="/template/header.jsp" %>
    ...
    <div class="rounded" style="background: #e6e6e6; padding: 10px;">
        <font class="formfieldnamefont" style="color: #666666">A few notes about joined <%=Pagez._surveys()%>:</font>
        <font class="smallfont" style="color: #666666;">
            <ul>
                <li><b>You get credit for <%=Pagez._surveys()%> when you generate clicks/displays for <%=Pagez._surveys()%> in the days after you take it.</b>  The little squares represent days you have to generate clicks.  Underneath the squares is a summary of your status.</li>
                ...
                <li><b>The square on the left represents the day that you join the <%=Pagez._survey()%>.</b>  Squares to the right of that square are the days after that.</li>
                <li><b>All <%=Pagez._survey()%> status boxes will automatically update overnight.</b>  Or, you can click the Refresh button to update whenever you'd like to check your status.</li>
...
"        </div>");
    ...
    <%if (bloggerCompletedsurveys.getList()==null || bloggerCompletedsurveys.getList().size()==0){%>
        <font class="normalfont">You haven't yet joined any <%=Pagez._surveys()%>!</font>
    <%} else {%>
        <%
    ...
    <% if (!Pagez.getUserSession().getIsfacebookui()){ %>
        ...
        Your blog posting script: Click on any <%=Pagez._survey()%> title to access the script that you'll use to post the <%=Pagez._survey()%> to your blog.
        <br/><br/>
