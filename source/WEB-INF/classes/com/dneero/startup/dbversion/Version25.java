 ...
 * Time: 11:57:46 AM
 */
public class Version25 implements UpgradeDatabaseOneVersion {
    ...
    public void doPostHibernateUpgrade(){
        ...
        //-----------------------------------
        //-----------------------------------
        int countdsa = Db.RunSQLUpdate("UPDATE user SET isfacebookappremoved=false");
        //-----------------------------------
        //-----------------------------------

        //-----------------------------------
        //-----------------------------------
        String[][] rstUser= Db.RunSQL("SELECT userid, createdate FROM user");
        //-----------------------------------
        //-----------------------------------
        if (rstUser!=null && rstUser.length>0){
            for(int i=0; i<rstUser.length; i++){
                //-----------------------------------
                //-----------------------------------
                int count = Db.RunSQLUpdate("UPDATE user SET facebookappremoveddate='"+ rstUser[i][1]+"' WHERE userid='"+rstUser[i][0]+"'");
                //-----------------------------------
                //-----------------------------------
            }
        }
