package com.dneero.cache.providers.dbcache;
...
//            }
    ...
    public void flush(String group) {
        Logger logger = Logger.getLogger(this.getClass().getName());
        try{
            HibernateUtilDbcache.getSession().createQuery("delete Dbcache d where d.dbcacheid>0 and d.grp='"+group+"'").executeUpdate();
        } catch (Exception ex){
            logger.error("", ex);
        }
    }
